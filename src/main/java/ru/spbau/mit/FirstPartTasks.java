...
public final class FirstPartTasks {
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums.flatMap(album -> album.getTracks()
                                            .stream()
                                            .map(Track::getName))
                     .sorted()
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Album::getName))
                     .entrySet()
                     .stream()
                     .mapToInt(value -> value.getValue().size() - 1)
                     .sum();
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparingDouble(a -> -a.getTracks().stream().mapToInt(Track::getRating).average().orElse(0))).collect(Collectors.toList());
    }
